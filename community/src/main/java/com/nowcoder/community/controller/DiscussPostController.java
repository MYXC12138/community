    ...
    public String getDiscussPost(@PathVariable("discussPostId") int discussPostId, Model model, Page page){
        ...
        if (commentList != null){
            for (Comment comment : commentList){
                // 评论VO
                Map<String, Object> commmentVo = new HashMap<>();
                // 评论
                commmentVo.put("comment",comment);
                // 作者
                commmentVo.put("user",userService.selectById(comment.getUserId()));
                // 点赞有关信息
                likeCount = likeService.findEntityLikeCount(ENTITY_TYPE_COMMENT,comment.getId());
                commmentVo.put("likeCount",likeCount);
                // 点赞状态
                likeStatus = hostHolder.getUser() == null ? 0:
                        likeService.findEntityLikeStatus(hostHolder.getUser().getId(),ENTITY_TYPE_COMMENT,comment.getId());
                commmentVo.put("likeStatus",likeStatus);
                ...
                commmentVo.put("replys", replyVoList);
                
                // 回复数量
                int replyCount = commentService.findCommentCountByEntity(ENTITY_TYPE_COMMENT, comment.getId());
                commmentVo.put("replyCount", replyCount);

                commentVoList.add(commmentVo);
            }
        }
